...
if __name__ == "__main__":   
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        # Now, receive the linecam Data message in a loop
        i = 0;
        while True:
            msg3 = recvmsg(s)
            if msg3 is None:
            ...
            if data.HasField("image") and data.image.HasField("yvals"):
                ...
                print("{}, {} : {}".format(i, data.image.shot_num, y1_vals[5]))
            if data.HasField("image") and data.image.HasField("shot_time"):
                print(datetime.datetime.fromtimestamp(data.image.shot_time.seconds, datetime.timezone.utc))

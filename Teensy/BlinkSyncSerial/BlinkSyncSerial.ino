/* BlinkSync
    ...
    PINS:
      ...
      19    |   Output, reference trigger at 20 Hz with 10% duty cycle (optional usage for internal triggering is to bridge pins 19 and 23). User-configurable frequency.
      3     |   Output, green status LED, heartbeat. Flashes once per second reliably.
      4     |   Output, blue status LED. Indicates "actively updating data." When lit, data should not be read.
...
uint32_t refFreqHz = 20; // reference / internal trigger frequency, in hertz
...
void setup() {
  ...
  analogWriteFrequency(REF_PIN, refFreqHz); // Set the frequency of the reference pulse
...
void loop() {
  ...
  if (await_update) {
      await_update = false;
      digitalWrite(REF_PIN, LOW); // turn off the reference pulse
      analogWriteFrequency(REF_PIN, refFreqHz); // Set the frequency of the reference pulse
      analogWrite(REF_PIN, 26); // Creates a reference pulse of duty cycle 26/255 = 10%
  }
